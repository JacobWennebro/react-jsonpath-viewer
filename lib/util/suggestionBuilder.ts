...
export const getSuggestions = (jsonPath: string, caretPosition: number, jsonToTestAgainst: object) => {
  try {
    ...
    if (Array.isArray(filteredJson)) {
      ...
      let additionalSuggestions: Array<Suggestion> = [];
      if (parentJsonPath) {
...
const guessSuggestionsFromScopes = (scopes: Array<Suggestion>, carretPosition: number, jsonPath: string, jsonToTestAgainst: object) => {
...
export const evalAllProperties = (suggestions: Array<Suggestion>, carretPosition: number, jsonPath: string, jsonToTestAgainst: object) => {
...
const getAllPropertiesRecursively = (objectOrArray: unknown | object | Array<unknown>): Array<string> | null => {
  ...
  } else if (typeof objectOrArray === 'object' && objectOrArray) {
    const keys = Object.keys(objectOrArray);
    const subkeys = keys.map(key => {
      return getAllPropertiesRecursively(objectOrArray[key as keyof typeof objectOrArray]);
    }).filter(k => k !== null);
...
const getAllPropertiesAtFirstLevel = (objectOrArray: object | Array<unknown>): Array<string> | null => {
  if (Array.isArray(objectOrArray)) {
    return flatten(objectOrArray.map(arrayEntry => {
